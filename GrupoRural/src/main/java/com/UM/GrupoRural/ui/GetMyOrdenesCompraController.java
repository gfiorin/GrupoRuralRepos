...
@RestController
@RequestMapping("/GetMyOrdenesCompra/{user}")
@CrossOrigin("*")
public class GetMyOrdenesCompraController {
    @Autowired
    UserMgr userMgr;

    @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> getGrupos(@PathVariable("user") String usuario) throws IOException {
        return ResponseEntity.ok(userMgr.findCompradorByUsuario(usuario).getOdenesDeCompra());
    }
}
